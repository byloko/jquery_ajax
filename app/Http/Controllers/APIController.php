	...
	public function app_user_list(Request $request){
	  $getresultCount = User::where('id', '=', $request->user_id)->count();
		...
		foreach ($getresult as $value) {
			...
			$option_main = array();
			$gerOrder = OrdersModel::where('user_id', '=', $value->id)->orderBy('id', 'desc')->get();
			foreach($gerOrder as $value_row){
				$data_x['id'] = $value_row->id;
				$data_x['user_id'] = $value_row->user_id;
				// order_details start
				$sub_option_main = array();
				$getOrderDetail = OrderDetailsModel::where('order_id', '=', $value_row->id)->orderBy('id', 'desc')->get();
				foreach($getOrderDetail as $value_x_row){
					$data_z['id'] = $value_x_row->id;
					$sub_option_main[] = $data_x;
				}
				$data_x['order_details_list'] = $sub_option_main;
				// order_details end
				$option_main[] = $data_x;
			}
			$data['order_list']    = $option_main;
			$result[] = $data;
		}
